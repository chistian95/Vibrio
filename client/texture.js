/*BICHO
======================================================================*/
tentaculo = PIXI.Texture.fromImage('assets/img/tent2.png');
ojo = PIXI.Texture.fromImage('assets/img/ojo.png');
pincho = PIXI.Texture.fromImage('assets/img/Pincho.png');

/*BLACKGROUND
======================================================================*/
backgroundFijo = PIXI.Texture.fromImage('assets/img/fondo.png');
parallax1 = PIXI.Texture.fromImage('assets/img/parallax1.png');
parallax2 = PIXI.Texture.fromImage('assets/img/parallax2.png');

/*UI
=====================================================================*/
uiPincho = PIXI.Texture.fromImage('assets/img/ico-pinchos.png');
uiOjo = PIXI.Texture.fromImage('assets/img/ico-ojos.png');
uiTentaculo = PIXI.Texture.fromImage('assets/img/ico-tentaculos.png');
uiZise = PIXI.Texture.fromImage('assets/img/ico-size.png');
uiNodos = PIXI.Texture.fromImage('assets/img/ico-nodos.png');
uiCoraza = PIXI.Texture.fromImage('assets/img/ico-coraza.png');
barraExp =  PIXI.Texture.fromImage('assets/img/expBar.png');
exp =  PIXI.Texture.fromImage('assets/img/exp.png');

/*PLANTAS
=====================================================================*/
